...
glue_answer = {
    "CoLA": {1:"correct", 0:"incorrect", -1:"none"},
    "SST2": {1:"positive", 0:"negative", -1:"none"},
    "MRPC": {1:"yes", 0:"no", -1:"none"},
    "QQP":  {1:"yes", 0:"no", -1:"none"},
    "MNLI": {2:"entailment", 1:"neutral", 0:"entailment", -1:"none"},
    "QNLI": {1:"contains", 0:"not contain", -1:"none"},
    "RTE":  {1:"not entail", 0:"entails", -1:"none"},
}
def main():
    ...
    if '.json' not in args.number_experts:
        number_experts = args.number_experts.split(",")
        number_experts = [int(lr) for lr in number_experts]
    else:
        number_experts = json.load(open(args.number_experts, 'r', encoding='utf-8'))
    ...
    for start_idx in tqdm(range(0, len(data_a), max_batch_size)):
        ...
        for example in batch:
            try:
                answers.append(re.findall(ans_pattern, str(example["answer"]))[0])
            except IndexError:
        ...
        for r_i in range(len(res)):         
            answer = res[r_i][0] if len(res[r_i])!=0 else '0'
            pred.append(answer)
            ...
            func = float
            try:
                func(answer) == func(answers[r_i])
            except ValueError:
                func = str

            if func(answer) == func(answers[r_i]):
                correct += 1
                print('correct:', func(answer), func(answers[r_i]))
            else:
                print('gt-ans:', func(answer), func(answers[r_i]))
