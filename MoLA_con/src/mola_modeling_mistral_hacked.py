...
from src.attn import _prepare_4d_causal_attention_mask, _prepare_4d_causal_attention_mask_for_sdpa
...
def load_balancing_loss_func(gate_logits: torch.Tensor, num_experts: torch.Tensor = None, top_k=2) -> float:
    ...
    for layer_i in range(len(gate_logits)):
        for layer_j in range(len(gate_logits[layer_i])):
            num_experts_dict = {
                0:'q_proj',
                1:'v_proj',
                2:'k_proj',
            ...
            if layer_j > 3:
                layer_i_expert = f"model.layers.{layer_i}.mlp.{num_experts_dict[layer_j]}"
            else:
                layer_i_expert = f"model.layers.{layer_i}.self_attn.{num_experts_dict[layer_j]}"
            layer_gate = gate_logits[layer_i][layer_j]
            ...
            expert_mask = torch.nn.functional.one_hot(selected_experts, num_experts[layer_i_expert])
            ...
            layer_loss = torch.sum(tokens_per_expert * router_prob_per_expert) * num_experts[layer_i_expert]
...
class MistralDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_router_logits: ##newmodified
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    output_router_logits=output_router_logits,#newmodified
                    use_cache=use_cache,
                )
...
class MistralForCausalLM_d(MistralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.router_aux_loss_coef = 0.001 ##newmodified
