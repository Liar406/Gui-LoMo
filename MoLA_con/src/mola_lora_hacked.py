...
class LoraMoE_Layer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, layer_index): ### modified2
        ...
        self.lora_dropout.update(nn.ModuleDict({adapter_name: lora_dropout_layer}))
        # Actual trainable parameters
        if isinstance(r, dict): ### modified2
            ##newmodified
            ...
            for i in range(self.number_experts):
                ...
                self.lora_A.update(nn.ModuleDict({adapter_name_moe: nn.Linear(self.in_features, r[layer_index + f'.expertlora_E_vector_{i}'], bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name_moe: nn.Linear(r[layer_index + f'.expertlora_E_vector_{i}'], self.out_features, bias=False)}))
            self.scaling[adapter_name] = lora_alpha / r[layer_index + f'.expertlora_E_vector_{i}']
        else:
            if r > 0:
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
...
class Linear_MoE(nn.Linear, LoraMoE_Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.top_k = top_k[int(layer_index.split(".")[2])] 
    ...
    def forward(self, x: torch.Tensor):
        ...
        #expert_mask.shape = [batch_size*sequence_length, topk, number_experts]
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.number_experts).permute(2, 1, 0)
        #expert_mask.shape = [number_experts, topk, batch_size*sequence_length]
        ...
        for expert_idx in range(self.number_experts):
            ...
            #idx:topk, top_x:batch_size*sequence_length
            if top_x.shape[0] == 0:
                continue
