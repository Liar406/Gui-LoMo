...
def load_balancing_loss_func(gate_logits: torch.Tensor, num_experts: torch.Tensor = None, top_k=2) -> float:
    ...
    if isinstance(gate_logits, tuple):
        tensor = gate_logits
        while not isinstance(tensor, torch.Tensor):
            tensor = tensor[0]
        compute_device = tensor.device
    # print(len(gate_logits))
    overall_loss = torch.tensor(0, dtype=torch.float32).to(compute_device)
    for layer_i in range(len(gate_logits)):
        # print(layer_i, len(gate_logits[layer_i]))
        for layer_j in range(len(gate_logits[layer_i])):
            ...
            # print(layer_i, num_experts_dict[layer_j], layer_gate.shape)
