...
@dataclass
class TrainingArguments_arch:
    arch_learning_rate: float = field(default=3e-4, metadata={"help": "The initial learning rate for AdamW."})
    betas:tuple = field(default=(0.5, 0.999), metadata={"help": "The initial betas for AdamW."})
    arch_weight_decay: float = field(default=1e-3, metadata={"help": "The arch_weight_decay for AdamW."})
    eps: float = field(default=1e-8, metadata={"help": "The eps for AdamW."})
...
def train(
    ...
    for name, parameters in model.named_parameters():
        print(f"{name}, :, {parameters.device},{parameters.requires_grad}")
    ...
    class SavePretrainedCallback(TrainerCallback):
        ...
        def __init__(self, save_at_epochs):
            super().__init__()
            self.save_at_epochs = save_at_epochs

        def on_epoch_end(self, args, state, control, model=None, **kwargs):
            current_epoch = int(round(state.epoch))
            print("current_epoch: ",current_epoch )# Round to nearest whole number
            if model is not None and current_epoch in self.save_at_epochs:
                output_dir = os.path.join(args.output_dir, f'checkpoint-{current_epoch}')
    ...
    trainer = Trainer(
        callbacks=[SavePretrainedCallback(save_at_epoch)],
        model=model,
        ...
        args_arch=TrainingArguments_arch(
